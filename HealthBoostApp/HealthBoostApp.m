...
static NSInteger HBWriteStepsToVarMobileDocuments(long steps) {
    ...
    // v2.2.4：写之前先删旧文件，打破缓存，确保微信能读到最新值
...
- (void)writeSamplesSequentially:(HKSourceRevision *)deviceRev
    ...
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:stepType
        ...
        // v2.0.2 修复（对应 Bug1/Bug2/Bug3）：
        //  · 无论用户当天是否已走路，都必须写入「合成步数样本(虚拟增量)」。
        //    旧代码仅在 hasRealDeviceSteps==YES 分支调用 writeVirtualStepSample，未走路时虚拟步数被整段跳过
        //    → 微信(CoreMotion)与健康(HealthKit)都拿不到虚拟步数（Bug1：微信步数≈0）。
        //  · 绝不删除真实设备/健康样本。旧 NO 分支会调 deleteObject 清掉设备样本，
        //    真实步数被抹掉后「最近30天」只剩合成虚拟值（Bug3：健康里只有虚拟、没有真实）。
        //    旧合成样本由 writeVirtualStepSample 按 metadata 自行清理，无需在此删除任何真实数据。
        //  · 距离/楼层以设备源样本写入（值为虚拟部分），真实距离来自设备自身样本，不重复累加。

        // 仅用于日志：检测是否已有真实设备步数（不影响写入逻辑）
            ...
            if (bid == nil || [bid hasPrefix:@"com.apple.health."]) {
                if ([s isKindOfClass:[HKQuantitySample class]]) {
                    ...
                    if (stepVal > 0) {
                        ...
                        HBLog(@"[UCS] 发现真实设备步数 %.0f（保留，仅叠加虚拟增量）", stepVal);
                        break;
                    }
                }
            }
        }
        HBLog(@"[UCS] hasRealDeviceSteps=%d；开始写入 合成步数样本 + 距离/楼层", hasRealDeviceSteps);

        // 1) 合成步数样本（=虚拟增量 v）：健康 App 读 Health 得 真实+虚拟；
        //    writeVirtualStepSample 内部按 metadata 删除旧合成样本，不会动真实数据。
        [self writeVirtualStepSample:steps];

        // 2) 距离 + 楼层（设备源样本，值为虚拟部分）
